...
Документ `ACCESS_CONTROL_AUDIT.md` **частично устарел** (товары там ещё Public). Актуально:
...
В `docs/user-manual-final.md` написано, что UI продаж и возвратов «в разработке». На практике кнопки есть в `ProductsView` — **руководство стоит обновить**, когда будет время.
...
| `ACCESS_CONTROL_AUDIT.md` | Products помечены как Public — **неактуально** |
