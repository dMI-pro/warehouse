...
Актуальный чеклист безопасности и статус задач — в `docs/BACKLOG-AND-SECURITY.md`.  
...
**Не закрыто (отдельная задача):** фото по URL `https://tsehh.ru/minio/...` — nginx отдаёт без авторизации. См. `BACKLOG-AND-SECURITY.md`, пункт про `/minio/`.
