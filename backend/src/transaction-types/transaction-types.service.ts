...
export class TransactionTypesService {
  ...
  async create(createDto: CreateTransactionTypeDto) {
    ...
    if (existing) {
      throw new ConflictException(`Transaction type "${createDto.name}" already exists`);
    }
  ...
  async update(id: number, updateDto: UpdateTransactionTypeDto) {
    const type = await this.prisma.transactionType.findUnique({ where: { id } });
    if (!type) {
      throw new NotFoundException(`Transaction type with ID ${id} not found`);
    }
    if (updateDto.name && updateDto.name !== type.name) {
      ...
      if (existing) {
        throw new ConflictException(`Transaction type "${updateDto.name}" already exists`);
      }
    }
  ...
  async remove(id: number) {
    ...
    if (type.products.length > 0) {
      throw new ConflictException('Cannot delete transaction type with associated products');
    }
